...
function harness(headers: Record<string, string> = {}) {
  ...
  // Every finished request is also a metric sample.
  const record = vi.fn();
  const metrics = { record } as unknown as MetricsService;

  return { request, response, setHeader, context, metrics, record };
}

describe('CorrelationInterceptor', () => {
  it('generates a correlation id and echoes it in the response header', async () => {
    const { context, request, setHeader, metrics } = harness();
    const interceptor = new CorrelationInterceptor(metrics);
  ...
  it('reuses an incoming correlation id, so a trace survives across services', async () => {
    const { context, request, metrics } = harness({ 'x-correlation-id': 'from-nginx' });
    const interceptor = new CorrelationInterceptor(metrics);
    await firstValueFrom(interceptor.intercept(context, { handle: () => of('ok') } as CallHandler));
  ...
  it('truncates an oversized incoming id rather than logging it whole', async () => {
    const { context, request, metrics } = harness({ 'x-correlation-id': 'x'.repeat(500) });
    const interceptor = new CorrelationInterceptor(metrics);
    await firstValueFrom(interceptor.intercept(context, { handle: () => of('ok') } as CallHandler));
